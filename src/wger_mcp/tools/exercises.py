...
from .common import api_list_tool, api_tool, as_int, bad_request, opt
...
def _shape_exercise(ex: dict[str, Any], query: str | None = None) -> dict[str, Any]:
    translations = [
    ...
    if query:
        ...
        match = translations[0] if translations else None
    return {
        "id": ex.get("id"),
        "uuid": ex.get("uuid"),
...
def register(mcp: FastMCP, api: AuthenticatedClient, settings: Settings) -> None:
    ...
    async def search_exercises(
        ...
        return [
            _shape_exercise(ex, query) | {"images": _shape_images(ex.get("images"))}
            for ex in results
        ]
    ...
    async def search_exercises_by_filter(
        ...
        return [_shape_exercise(ex) for ex in results]
