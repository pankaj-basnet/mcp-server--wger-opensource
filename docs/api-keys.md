...
| **Outbound** (MCP → wger) | `WGER_DEV_TOKEN` | this server | `static_token`, `none` |

**Never hand an outbound credential to a client.** `WGER_DEV_TOKEN` grants full
...
wger account behind `WGER_DEV_TOKEN`. Rotate by changing the variable and
restarting.
...
| Server starts, then every wger call fails | Outbound credential wrong: `WGER_DEV_TOKEN` invalid, or `WGER_BASE_URL` points at a different instance than the key belongs to. |
